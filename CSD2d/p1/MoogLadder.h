...
Special thanks to our lovely teachers: Pieter Suurmond, Ciska Vriezenga, Marc
Groenewegen
*/
...
struct MoogLadder {
  ...
  void setResonance(double resonance) {
    res = resonance;
    currentResonance = resonance * (1.0029f + (0.0526f * omega) -
                                    (0.0926f * pow(omega, 2.0f)) +
                                    (0.0218f * pow(omega, 3.0f)));
  }

  static LadderCoefficients makeLoPass12() noexcept {
    return {.A = 0, .B = 0, .C = 1, .D = 0, .E = 0};
  }

  static LadderCoefficients makeLoPass24() noexcept {
    return {.A = 0, .B = 0, .C = 0, .D = 0, .E = 1};
  }

  static LadderCoefficients makeBandPass12() noexcept {
    return {.A = 0, .B = 2, .C = -2, .D = 0, .E = 0};
  }

  static LadderCoefficients makeBandPass24() noexcept {
    return {.A = 0, .B = 0, .C = 4, .D = -8, .E = 4};
  }

  static LadderCoefficients makeHiPass12() noexcept {
    return {.A = 1, .B = -2, .C = 1, .D = 0, .E = 0};
  }

  static LadderCoefficients makeHiPass24() noexcept {
    return {.A = 1, .B = -4, .C = 6, .D = -4, .E = 1};
  }

private:
  struct MoogComponent {
    double process(double input) { return feedback(feedforward(input)); }

    void calculateG(double omega) noexcept {
      g = (0.9892 * omega) - (0.4342 * std::pow(omega, 2.0)) +
          (0.1381 * std::pow(omega, 3.0)) - (0.0202 * std::pow(omega, 4.0));
    }
