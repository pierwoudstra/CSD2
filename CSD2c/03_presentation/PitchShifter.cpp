...
void PitchShifter::applyEffect(const float &input, float &output) {
  ...
  // read fractional readHead and generate 0 and 180 degree readHead in integer form
  ...
  if (overlap >= (writeHead - readHeadInt1) && (writeHead - readHeadInt1) >= 0 && shift != 1.f) {
  ...
  if (overlap >= (writeHead - readHeadInt2) && (writeHead - readHeadInt2) >= 0 && shift != 1.f) {
  ...
  readHead += shift;
  writeHead++;
  if (writeHead == bufSize) writeHead = 0;
  if (int(readHead) >= bufSize) readHead = 0.f;
