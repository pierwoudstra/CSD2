...
/// This struct contains pointers to the input and output buffers provided by
/// Jack, as well as the number of channels in each buffer and the number of
/// frames.
struct AudioBuffer {
...
/// Base class for your own audio callback. Make a subclass of this and register
/// it with a `JackModule` instance.
class AudioCallback {
public:
  /// This function is called before process, so you can prepare any of your
  /// effects.
  virtual void prepare(int samplerate) {}

  /// This function is called when Jack requests new audio data. An instance of
  /// `AudioBuffer` is passed as an argument, which can be used to collect
  /// incoming audio and send outgoing sample data.
  virtual void process(AudioBuffer buffer) {}
};

/// Jack Client. Make an instance of this (only one per program) and provide it
/// with a reference to your `AudioCallback` subclass via its constructor. After
/// that call `init()` to start the Jack session.
class JackModule {
public:
  ...
  /// Call `init()` and pass the number of inputs and outputs. If no other
  /// arguments are passed the defaults mentioned below will be passed
  /// automatically. It is possible to manually enter arguments for `clientName`
  ...
  /// Returns the sampling rate at which Jack is running. Only call this after
...
private:
  ...
  static_assert(std::is_same_v<jack_default_audio_sample_t, float>,
                "this code assumes the jack audio sample type to be float, "
                "which is not the case if this assert fails");
  ...
  int onProcess(jack_nframes_t numFrames) {
    for (int channel = 0; channel < numInputChannels; ++channel) {
      inputBuffers[channel] = reinterpret_cast<float *>(
          jack_port_get_buffer(inputPorts[channel], numFrames));
    }

    for (int channel = 0; channel < numOutputChannels; ++channel) {
      outputBuffers[channel] = reinterpret_cast<float *>(
          jack_port_get_buffer(outputPorts[channel], numFrames));
    }
  ...
  static auto makePortsPtr(const char **ports) -> UniquePortsPtr {
    return {ports, [](auto **p) { free(p); }};
  }

  auto findPorts(std::string_view clientName, uint64_t flags)
      -> UniquePortsPtr {
    if (auto ports = makePortsPtr(
            jack_get_ports(client, clientName.data(), nullptr, flags))) {
      return ports;
    }

    throw std::runtime_error{"Cannot find capture ports associated with " +
    ...
    if (numInputChannels > 0) {
      ...
      if (countPorts(ports.get()) < numInputChannels) {
        throw std::runtime_error{
            "Not enough Jack ports for the number of requested input channels"};
      }

      for (int channel = 0; channel < numInputChannels; ++channel) {
        if (jack_connect(client, ports.get()[channel],
                         jack_port_name(inputPorts[channel]))) {
  ...
  void connectOutputs(std::string_view outputClient) {
    if (numOutputChannels > 0) {
      ...
      if (countPorts(ports.get()) < numOutputChannels) {
        throw std::runtime_error{"Not enough Jack ports for the number of "
                                 "requested output channels"};
      }

      for (int channel = 0; channel < numOutputChannels; ++channel) {
        if (jack_connect(client, jack_port_name(outputPorts[channel]),
                         ports.get()[channel])) {
  ...
  void registerInputPorts() {
    ...
    for (int channel = 0; channel < numInputChannels; ++channel) {
      ...
      const auto port = jack_port_register(
  ...
  void prepareCallback() { callback.prepare(getSampleRate()); }
  ...
  static void onJackShutdown(void *) { exit(1); }
};
